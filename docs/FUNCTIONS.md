...
**交付物声明 artifacts（§4.5b，2026-09-11 补）**：证据可带可选 `artifacts?: Artifact[]`（`{path 绝对路径, description, howVerified}`，≤10 条）——结构化指认**产物本体**，回答「产物是什么、在哪、内容是什么」；此前交付物只是 changesSummary 自由文本里的路径碎片，验收台看不到也不可预览。约定：
- **终检必须声明**：有文件/目录形态交付物时，终检会话逐项声明（终检本就只读核验，顺手 stat 确认存在性），验收台任务级判定面顶部即出现「交付物（n）」区——先看产物，再看判定；
- **子任务举证同样可带**：执行会话经 `taskflow_submit_evidence` 的可选 artifacts 参数声明；

...
- **入口**：看板 review 列卡片「验收」按钮 / 顶部「待验收」角标 / 详情抽屉。
- **审查页**（详情抽屉的 review 态）：上=合同（目标+验收），中=证据报告卡（verification 可展开输出全文），下=操作区。
- **批准**：单卡批准；全部子任务 done 时任务级自动 T6。（M1 任务级与子任务级判定合并为同一组点击；M2 提供任务级一次性批准全部。）
- **打回**：
  - 作用域：选中的子任务（默认 = selfCheck 含 partial/fail 的子任务）；
  - 批语**必填**（多行输入，UI 强提示「批语将原文注入下一轮执行」）；
  - 动作序列：子任务 → rejected（落事件：actor=human, reason=批语）→ 立即 in-progress 重新入队；round+1；任务若在 review 则回 in-progress（T7）；
  - 达 `maxRounds` 时打回按钮变为「已达迭代上限」：允许「提高上限并打回」或「转 blocked」。

### 4.7 迭代循环（FR-08）

- 轮次语义：子任务 `round` 从 1 起；每次打回 +1；卡片角标「第 n 轮」。
- 下一轮执行的注入 = 合同 + **批语原文** + 上轮证据摘要（引用 id，UI 可展开）。
- 跨会话文本注入一律带来源声明包装（安全模型见 §7）：「以下内容来自人类验收批语 / AI 上轮自检，未经本会话审阅」。
...
3. **动效克制**：仅三类——卡片 hover 抬升（translateY 1px + 阴影）、抽屉滑入（200ms ease-out）、状态变更的徽标脉冲一次；全部动效尊重 `prefers-reduced-motion`。
